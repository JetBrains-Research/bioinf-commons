...
class DataConfigTest {
    ...
    fun consistency() {
        withConfig { config ->
            assertTrue(DataConfig.Key("H3K4me3", CellId("test1")) in config.tracksMap)
            ...
            assertTrue(DataConfig.Key("H3K27ac", CellId("test0")) in config.tracksMap)
            assertTrue(DataConfig.Key("H3K9me3", CellId("test1")) in config.tracksMap)
            assertTrue(DataConfig.Key("H3K4me3", CellId("test2")) in config.tracksMap)
        }
    }
    ...
    fun technicalReplicate() {
        withConfig { config ->
            config.tracksMap[DataConfig.Key("H3K9me3", CellId("test1"))]!!.forEach {
    ...
    fun metaConsistency() {
        withConfig { config ->
            val section = config.tracksMap[DataConfig.Key("H3K27ac", CellId("test0"))]
    ...
    fun labeledConsistency() {
        withConfig { config ->
            val section = config.tracksMap[DataConfig.Key("H3K4me3", CellId("test1"))]
            assertEquals(setOf("rep1", "rep2"), section!!.map { it.first }.toSet())
        }
    }
    ...
    fun implicitConsistency() {
        withConfig { config ->
            val section = config.tracksMap[DataConfig.Key("H3K4me3", CellId("test2"))]
...
"""
            ...
            assertEquals(DataConfig.Key(DataType.METHYLATION.id, CellId("t")), dc.tracksMap.keys.first())
        }
    }
...
"""
            ...
            assertEquals(1, dc.tracksMap.size)
            assertEquals(1, dc.tracksMap.keys.size)
            assertEquals(DataConfig.Key(DataType.TRANSCRIPTION.id, CellId("t")), dc.tracksMap.keys.first())
        }
    }
...
"""
    ...
    fun testFailedTrack() {
        withConfig { config ->
            assertTrue(config.tracksMap[DataConfig.Key("H3K27ac", CellId("test0"))]!![0].second.failedTrack)
        }
    }
