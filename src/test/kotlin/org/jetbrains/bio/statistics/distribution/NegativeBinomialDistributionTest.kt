...
class NegativeBinomialDistributionTest {
    ...
    fun testMLE() {
        ...
        for (run in 0 until numRuns) {
            ...
            val nbSampling = NegativeBinomialDistribution.usingSuccessProbability(successProbability, numFailures.toDouble())
    ...
    fun testMLEF64Array() {
        ...
        for (run in 0 until numRuns) {
            ...
            try {
                ...
                val failures = NegativeBinomialDistribution.fitNumberOfFailures(samplesArray, weights, mean, failuresGuess)
    ...
    @Test
    fun testFixedValues() {
        val failures = doubleArrayOf(10.344394, 6.636840, 8.272800, 5.790573, 15.066386, 8.219735, 7.611640,
                                     5.793857, 8.699888, 14.319623)
        val successProbabilities = doubleArrayOf(0.60400253, 0.65120170, 0.03357039, 0.20654823, 0.60348266,
                                                 0.54437009, 0.57465349, 0.60595111, 0.39196646, 0.29655913)
        val means = doubleArrayOf(15.7779795, 12.3908904, 0.2873682, 1.5073790, 22.9304040, 9.8206412,
                                  10.2835114, 8.9095387, 5.6083489, 6.0369179)
        val sample = intArrayOf(1, 6, 5, 0, 4, 2, 4, 2, 4, 6)
        val probabilities = doubleArrayOf(4.306413e-04, 5.092278e-02, 2.920415e-05, 2.619186e-01,
                                          3.653011e-04, 1.754760e-02, 4.536087e-02, 3.277967e-02,
                                          1.370312e-01, 1.345970e-01)
        val logProbabilities = doubleArrayOf(-7.750235, -2.977445, -10.441200, -1.339722, -7.914789,
        ...
        for (i in 0..9) {
            ...
            assertEquals(logProbabilities[i],
