...
* ${DataType.values().sorted().joinToString(", ")}

Supported file formats:
- *.bam, *.bed, *.bed.gz, *.bed.zip for ChIP-Seq
- *.bam for BS-Seq
...
"""
    ...
    fun getCellIds(dataType: DataType): Collection<CellId> =
            tracksMap.keys.filter { it.dataType.toDataType() == dataType }.map(Key::cellId).distinct()

    data class Key(val dataType: String, val cellId: CellId)
...
data class ReplicateData constructor(val path: Path, internal val meta: Map<String, Any> = emptyMap()) {
    ...
    companion object {
        ...
        val BIGWIG_KEY = ReplicateDataKey("bw") {
            it?.toString()?.toPath()
        }
